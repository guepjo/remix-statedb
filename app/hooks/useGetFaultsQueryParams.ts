 ...
 */
export const useGetFaultsQueryParams = (): [GetFaultsURLQueryParams, SetSearchParams] => {
  if (window.location.pathname !== '/search') {
    throw new Error(`useGetFaultsQueryParams can called on the ${window.location.host}/search path`);
  }

  const [searchParams, setSearchParams] = useSearchParams();
  const existingURLSearchParams = Object.fromEntries(searchParams) as GetFaultsURLQueryParams;
