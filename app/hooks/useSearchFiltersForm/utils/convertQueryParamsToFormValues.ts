import { FormattedQueryParamTypes, GetFaultsQueryParams } from 'hooks/useGetFaults';
 ...
 */
export const covertQueryParamsToFormValues = (urlSearchParamsMap: QueryParams) => {
  const result: GetFaultsQueryParams = {};

  if (urlSearchParamsMap?.actionable) {
    result.actionable = urlSearchParamsMap.actionable;
  }

  if (urlSearchParamsMap?.application) {
    result.application = urlSearchParamsMap.application;
  }

  if (urlSearchParamsMap?.check_id) {
    result.check_id = urlSearchParamsMap.check_id;
  }
  ...
  if (urlSearchParamsMap.date_filter) {
    const dateFilter = JSON.parse(urlSearchParamsMap.date_filter as string) as DateFilterType;
    ...
    if (qualifier === 'between' && Array.isArray(value)) {
      ...
      const startDateUTCToLocalDate = convertUtcDateToLocalMomentObject(startDate);
      ...
      const startDateUTCToLocalDate = convertUtcDateToLocalMomentObject(value as string);

      result.date_filter_after = startDateUTCToLocalDate;
    } else if (qualifier === 'before') {
      const endDateUTCToLocalDate = convertUtcDateToLocalMomentObject(value as string);
