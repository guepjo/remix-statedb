 ...
 */
export const useSearchFiltersForm = (formInstance: FormInstance<any>) => {
  const faultsMetaDataInfo = useGetFaultsMetaData();
  const { currentUser } = useAuth();
  const [urlSearchParamsMap] = useGetFaultsQueryParams();
  const [isSearchFiltersFormOpen, setIsSearchFiltersFormOpen] = React.useState(false);
  const [isGetFaultsHookEnabled, setIsGetFaultsHookEnabled] = React.useState(false);
  const [isMyFaultsSwitchButtonEnabled, setIsMyFaultsSwitchButtonEnabled] = React.useState(
    urlSearchParamsMap.created_by === currentUser.username,
  );
  const [faultsQueryParams, setFaultsQueryParams] = React.useState({} as GetFaultsQueryParams);
  const formattedFormValuesMap = covertQueryParamsToFormValues(urlSearchParamsMap);
   ...
   */
  const formFieldKeyValuePairs = Object.entries(formattedFormValuesMap).map((formField) => {
    return {
      name: formField[0],
      value: formField[1],
    };
  ...
  const clearForm = () => {
    ...
    formInstance.resetFields(formFields);
    notification.info({
  ...
  const handleFormSubmit = (newFaultFormValues: GetFaultsQueryParams) => {
    const formattedNewFaultFormValues = formatFormDataOnSubmit(newFaultFormValues);
   ...
   */
  formInstance.setFields(formFieldKeyValuePairs);
